...
- 학생이 정답을 하나도 마킹하지 않고 제출하는 경우도 있을 수 있어, 기존에 답안이 최소 1개는 있어야 통과되던 서버 로직을 수정해 빈 정답 배열도 에러 없이 제출되도록 했습니다.
- 채점 결과는 맞은 개수, 틀린 개수, 미응답 개수가 각각 나뉘어 내려오기 때문에, 답안을 하나도 마킹하지 않은 제출도 정상적인 케이스로 처리하는 것이 맞다고 판단했습니다.
